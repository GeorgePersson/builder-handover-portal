...
- 2026-06-22: Added first-pass request-more-context classification for Docling spec rows. Extraction still returns 96 proposals, but actions now split into standard review plus `needs_model_code`, `needs_source_document`, and `request_more_context`; latest 100-row upload was backfilled with review notes (32 context-request rows visible via notes). Lint/build passed; direct enum migration via Supabase CLI was blocked by local 5432 network access, so runtime falls back to `admin_review` status plus explicit review notes until `docs/supabase-add-extracted-item-context-statuses.sql` is applied.
